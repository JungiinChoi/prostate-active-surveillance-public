...
V.PGG.mri <- as.matrix(ind_mri_data[pgg_patient_index_map])

V.PGG.stage1 <- matrix(nrow=npat_pgg, ncol=1)
for(j in 1:npat_pgg){
  if(pgg_patient_index_map[j] <= npat_cancer_known){
    V.PGG.stage1[j,1] <- (eta.data[pgg_patient_index_map[j]]>2.5) * ind_mri_data[pgg_patient_index_map[j]]
  }else{
    V.PGG.stage1[j,1] <- pred_eta[(pgg_patient_index_map[j]-npat_cancer_known),1]>2.5) * ind_mri_data[pgg_patient_index_map[j]]
  }
}
...
V.ETA.data.obs <- modmat_cancer[inx_eta_obs,]
#### sequential replaced model
cancer_int1 <- read_csv("generated-files-seq-mri-form2/jags-prediction-cancer_int1-2022.csv")
cancer_int2 <- read_csv("generated-files-seq-mri-form2/jags-prediction-cancer_int2-2022.csv")
cancer_int3 <- read_csv("generated-files-seq-mri-form2/jags-prediction-cancer_int3-2022.csv")

cancer_slope1 <- read_csv("generated-files-seq-mri-form2/jags-prediction-cancer_slope1-2022.csv")
cancer_slope2 <- read_csv("generated-files-seq-mri-form2/jags-prediction-cancer_slope2-2022.csv")
cancer_slope3 <- read_csv("generated-files-seq-mri-form2/jags-prediction-cancer_slope3-2022.csv")
