...
foldnum = 6 #K-fold CV (K)


#User will need to edit
#This uses CV data and directory locations
base.location <- "/Users/user/Documents/GitHub/prostate-active-surveillance-vDaan/"
location.of.data <- paste0(base.location, "data", sep="") 
location.of.r.scripts <- paste0(base.location, "code_UCSF", sep="")
location.of.generated.files <- paste0(base.location, "generated-files", sep="")
location.of.generated.folder <- paste(location.of.generated.files, "/UCSF_K_",foldnum,sep="")
...
# plot and summary gamma


pgg_slope1 <- apply(as.matrix(read.csv(paste0(location.of.generated.folder, 
                                              "/jags-prediction-pgg_slope1-both10.csv")))[,-1],1,mean)

pgg_slope2 <- apply(as.matrix(read.csv(paste0(location.of.generated.folder, 
                                              "/jags-prediction-pgg_slope2-both10.csv")))[,-1],1,mean)

pgg_slope3 <- apply(as.matrix(read.csv(paste0(location.of.generated.folder, 
                                              "/jags-prediction-pgg_slope3-both10.csv")))[,-1],1,mean)

gamma_df <- data.frame(gamma1 = pgg_slope1, gamma2 = pgg_slope2, gamma3 = pgg_slope3)

ggplot(gamma_df) +
  geom_histogram(aes(x = pgg_slope1), color="#e9ecef", alpha=0.6) +
  theme_minimal() +
  geom_vline(xintercept = mean(pgg_slope1), color = "red") +
  labs(x="Gamma_1", y = "")

ggplot(gamma_df) +
  geom_histogram(aes(x = pgg_slope2), color="#e9ecef", alpha=0.6) +
  theme_minimal() +
  geom_vline(xintercept = mean(pgg_slope2), color = "red") +
  labs(x="Gamma_2", y = "")

ggplot(gamma_df) +
  geom_histogram(aes(x = pgg_slope3), color="#e9ecef", alpha=0.6) +
  theme_minimal() +
  geom_vline(xintercept = mean(pgg_slope3), color = "red") +
  labs(x="Gamma_3", y = "")

quantile(pgg_slope1, p = c(0.05,0.95))
quantile(pgg_slope2, p = c(0.05,0.95))
quantile(pgg_slope3, p = c(0.05,0.95))
...
obs.eta <- obs.eta[fitted.eta.index]
...
eta.true <- dx.ucsf$rp[!is.na(dx.ucsf$rp)]
...
N <- 360
#get eta predictions
ptdata_cvgroup <- dx.ucsf$cvgroup[1:N] ## 04/15/22 Zitong
fitted.eta<-matrix(nrow=B, ncol=N)
for(group in 1:foldnum){
  obs_eta_cv <- read.csv(paste0("/Users/user/Documents/GitHub/prostate-active-surveillance-vDaan/generated-files/UCSF_K_10", "/eta-subj-",group,".csv"))
	fit_eta_cv <- read.csv(paste0("/Users/user/Documents/GitHub/prostate-active-surveillance-vDaan/generated-files/UCSF_K_10", "/jags-prediction-eta.track-both",group,".csv"))
	fit_eta_cv <- fit_eta_cv[,-1]
	group_inx <- which(dx.ucsf$subj %in% obs_eta_cv$subj)
	print(group_inx)
	for(icol in 1:length(group_inx)){
	  fitted.eta[,group_inx[icol]] <- fit_eta_cv[,icol]
	}
} 
fitted.eta.index <- !is.na(fitted.eta[1,])
N <- sum(fitted.eta.index)
fitted.eta = fitted.eta[,fitted.eta.index]
...
rer <- roc(fitted.inf.obs[,1], obs.eta.mat[,1])
plot(rer)
...
y<- fitted.inf.obs[,1]
data<-data.frame(x,y)
names(data)<-c("Observed","Fitted")
roc.plot(data$Observed, data$Fitted)

library(pROC)
roc_score=roc(x, y) #AUC score
plot(roc_score ,main ="ROC curve -- Logistic Regression ")

plot(prediction( fitted.inf.obs[,j], obs.eta.mat[,j]))

pred <- prediction( fitted.inf.obs[,3], obs.eta.mat[,3])
perf <- performance(pred, "tpr", "fpr")
plot(perf,
     avg= "threshold",
     colorize=TRUE,
     lwd= 3, main = "True cancer state > 3 or not")
