...
pgg_int2 ~ dnorm(pgg_int2_mean, 1)
pgg_int3 ~ dnorm(pgg_int3_mean, 1)
for(index in 1:npred_pgg) {
...
for(j in 1:npat){ 
  exponent1[j] <- cancer_int1[j] + inprod(cancer_slope1[1:npred_cancer], modmat_cancer[j,1:npred_cancer])
  exponent2[j] <- cancer_int2[j] + inprod(cancer_slope2[1:npred_cancer], modmat_cancer[j,1:npred_cancer])
  exponent3[j] <- cancer_int3[j] + inprod(cancer_slope3[1:npred_cancer], modmat_cancer[j,1:npred_cancer])
...
if (mri_role %in% c("moderator", "both")){
  if (mri_st){
    "+ pgg_slope2[(npred_pgg + 4)] * (pgg_pirads_data_m2[pgg_patient_index_map[j]]) * (step(eta[pgg_patient_index_map[j]] - 2)-(1-step(eta[pgg_patient_index_map[j]] - 2))) * step(pgg_pirads_data[pgg_patient_index_map[j]] - 3)  
    ## logitP(pgg = 1) w/  (pirads - 2)*[1(eta >= 2)-1(eta=1)]*1(pirads >= 3)"
  } else{
    "+ pgg_slope2[(npred_pgg + 4)] * (step(eta[pgg_patient_index_map[j]] - 2)-(1-step(eta[pgg_patient_index_map[j]] - 2))) * step(pgg_pirads_data[pgg_patient_index_map[j]] - 3)  
    ## logitP(pgg = 1) w/  (pirads - 2)*[1(eta >= 2)-1(eta=1)]*1(pirads >= 3)"
  }
},
"
...
if (mri_role %in% c("moderator", "both")){
  if (mri_st){
    "+ pgg_slope3[(npred_pgg + 4)] * (pgg_pirads_data_m2[pgg_patient_index_map[j]]) * (step(eta[pgg_patient_index_map[j]] - 2)-(1-step(eta[pgg_patient_index_map[j]] - 2))) * step(pgg_pirads_data[pgg_patient_index_map[j]] - 3)  
    ## logitP(pgg = 1) w/  (pirads - 2)*[1(eta >= 2)-1(eta=1)]*1(pirads >= 3)"
  } else{
    "+ pgg_slope3[(npred_pgg + 4)] * (step(eta[pgg_patient_index_map[j]] - 2)-(1-step(eta[pgg_patient_index_map[j]] - 2))) * step(pgg_pirads_data[pgg_patient_index_map[j]] - 3)  
    ## logitP(pgg = 1) w/  (pirads - 2)*[1(eta >= 2)-1(eta=1)]*1(pirads >= 3)"
  }
},
"
...
for (index in 1:(nlevel_cancer - 1)) {
...
for(j in 1:npat_pirads){ 
  pirads_exp1[j] <- pirads_int1 +
	               pirads_slope1[1] * step(eta[pirads_patient_index_map[j]] - 2) + 
	               pirads_slope1[2] * step(eta[pirads_patient_index_map[j]] - 3) + 
	               pirads_slope1[3] * step(eta[pirads_patient_index_map[j]] - 4)
  pirads_exp2[j] <- pirads_int2 +
	               pirads_slope2[1] * step(eta[pirads_patient_index_map[j]] - 2) + 
	               pirads_slope2[2] * step(eta[pirads_patient_index_map[j]] - 3) + 
	               pirads_slope2[3] * step(eta[pirads_patient_index_map[j]] - 4)
