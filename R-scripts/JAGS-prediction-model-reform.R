...
for(k in 1:(K-1)){cancer_intercept0[k] ~ dnorm(0,0.01)}
cancer_intercept[1:(K-1)] <- sort(cancer_intercept0)
#proportional odds coefficients
for(index in 1:d.V.ETA){cancer_slope[index] ~ dnorm(0,0.01)}
...
for(i in 1:n){
  lin.pred.eta[i] <- inprod(cancer_slope[1:d.V.ETA], V.ETA[i,1:d.V.ETA])
