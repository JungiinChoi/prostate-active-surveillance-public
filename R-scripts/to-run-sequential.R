...
location.of.generated.files <- paste0(base.location, "generated-files-seq")
...
source(paste(location.of.r.scripts,"argument-prep-for-jags-seq.R",sep="/"))

#Define JAGS model
source(paste(location.of.r.scripts,"JAGS-prediction-model-seq.R",sep="/"))
...
outj<-jags(jags_data, inits=inits, parameters.to.save=params,
           model.file=paste(location.of.r.scripts, 
                            "JAGS-prediction-model-seq.txt", sep="/"),
