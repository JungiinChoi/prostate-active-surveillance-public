...
location.of.generated.files <- paste0(base.location, "generated-files-cancer-model/sequential_model/cv")
...
## cross validate
idlist1 <- unique(pt.data.eta$clinical_PTnum[pt.data.eta$true.pgg==1])
idlist2 <- unique(pt.data.eta$clinical_PTnum[pt.data.eta$true.pgg==2])
idlist3 <- unique(pt.data.eta$clinical_PTnum[pt.data.eta$true.pgg==3])
idlist4 <- unique(pt.data.eta$clinical_PTnum[pt.data.eta$true.pgg==4])
set.seed(2022)
...
coef_var_opt <- "sigmaj"  #sigma, sigmaj
model.file = paste0("code/jags_model/cancerreg_seqJAGS_",
                    coef_mean_opt, "_", coef_var_opt,".txt")
location.of.generated.files <- 
  paste0(base.location, 
         "generated-files-cancer-model/sequential_model/cv/", 
         coef_mean_opt, "_", coef_var_opt)

for(k in 1:5){
  message(k)
  test_id_stage1 = c(test_ids1[[k]],test_ids2[[k]],test_ids3[[k]],test_ids4[[k]])
  train_pos1 = which(!(pt.data.eta$clinical_PTnum[pt.data.eta$true.pgg>=1] %in% test_id_stage1))
  cancer_data1 = cancer_data[train_pos1]; cancer_data1 = ifelse(cancer_data1 == 1, 1, 0)
  modmat_cancer1 = modmat_cancer[train_pos1,]
  npat_cancer1 = length(cancer_data1)
  
  test_id_stage2 = c(test_ids2[[k]],test_ids3[[k]],test_ids4[[k]])
  train_pos2 = which(!(pt.data.eta$clinical_PTnum[pt.data.eta$true.pgg>=2] %in% test_id_stage2))
  inx_lev2 <- which(cancer_data > 1)
  cancer_data2 = cancer_data[inx_lev2]; cancer_data2 = cancer_data2[train_pos2]
  cancer_data2 = ifelse(cancer_data2 == 2, 1, 0)
  modmat_cancer2 = modmat_cancer[inx_lev2,]; modmat_cancer2 = modmat_cancer2[train_pos2,]
  npat_cancer2 = length(cancer_data2)
  
  test_id_stage3 = c(test_ids3[[k]],test_ids4[[k]])
  train_pos3 = which(!(pt.data.eta$clinical_PTnum[pt.data.eta$true.pgg>=3] %in% test_id_stage3))
  inx_lev3 <- which(cancer_data > 2)
  cancer_data3 = cancer_data[inx_lev3]; cancer_data3 = cancer_data3[train_pos3]
  cancer_data3 = ifelse(cancer_data3 == 3, 1, 0)
  modmat_cancer3 = modmat_cancer[inx_lev3,]; modmat_cancer3 = modmat_cancer3[train_pos3,]
  ...
  inits <- function() {
    ...
    if(coef_var_opt == "sigma"){
      ...
      inv_sigma_sq = rgamma(npred_cancer, 1, 1)
    }
    if(coef_mean_opt == "betaj"){
      coef_mean = rnorm(npred_cancer,mean=0,sd=0.25)
      list(
  ...
  params <- c(
    ...
    "cancer_slope1", "cancer_slope2", "cancer_slope3", "inv_sigma_sq",
    "sigma_sq"
  )
...
for(k in 1:5){
  jags_int1 <-
    read_csv(paste(location.of.generated.files, "/jags_cancer_int1_",seed,"cv",k,".csv",sep=""))
  jags_int2 <-
    read_csv(paste(location.of.generated.files, "/jags_cancer_int2_",seed,"cv",k,".csv",sep=""))
  jags_int3 <-
    read_csv(paste(location.of.generated.files, "/jags_cancer_int3_",seed,"cv",k,".csv",sep=""))
  jags_slope1 <-
    read_csv(paste(location.of.generated.files, "/jags_cancer_slope1_",seed,"cv",k,".csv",sep=""))
  jags_slope2 <-
    read_csv(paste(location.of.generated.files, "/jags_cancer_slope2_",seed,"cv",k,".csv",sep=""))
  jags_slope3 <-
    read_csv(paste(location.of.generated.files, "/jags_cancer_slope3_",seed,"cv",k,".csv",sep=""))
  ...
  test_pos = which(pt.data.eta$clinical_PTnum %in% test_id_all)
  modmat_cancer_test = modmat_cancer[test_pos,]
  cancer_data_test = pt.data.eta[test_pos,]
  
  p_rc1 <-p_rc2 <-p_rc3 <- p_rc4 <-matrix(0, ncol = nsim, nrow = dim(modmat_cancer_test)[1])
  for(i in 1:nsim){
    ...
    linpred1 <- modmat_cancer_test %*% slope1
    p_y1_given_x0 <- expit(matrix(int1 + c(linpred1)))
    linpred2 <- modmat_cancer_test %*% slope2
    p_y2_given_x0ge1 <- expit(matrix(int2 + c(linpred2)))
    linpred3 <- modmat_cancer_test %*% slope3
  ...
  obs1 <- sum(cancer_data_test$true.pgg == 1);obs2 <- sum(cancer_data_test$true.pgg == 2)
  obs3 <- sum(cancer_data_test$true.pgg == 3);obs4 <- sum(cancer_data_test$true.pgg == 4)
...
set.seed(2022); prior_distr <- rinvgamma(750,1,1)
...
set.seed(2022); prior_distr <- rnorm(750,0,1)
#get posterior distribution
variable_extract <- "cancer_slope3"
coef_seq <- c(#"betaj(k-1)_sigmaj", "betaj(k-1)_sigma",
              "betaj_sigmaj", "betaj_sigma")
jags_post_allmodel <- list()
for(i in 1:length(coef_seq)){
...
par(mfrow = c(1,2), oma = c(0,0,0,0), mar = c(2,2,2,2))
for(icol in 1:2){
  yrange = range(c(density(prior_distr)$y, 
  ...
  lines(density(jags_post_allmodel[[2]][, icol]), col = 'lightblue', lty = 2)
   legend(x = "topright",          # Position
         ...
         col = c("grey", "lightblue", "lightblue"),           # Line colors
         lwd = 1,
         cex= 0.8)  
}
