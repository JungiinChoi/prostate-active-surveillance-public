...
# 
# # add mri to pgg data prep section
# tmp_dt <- pt.data %>% 
...
# 
# # add pirads score to pgg data prep section
...
library(tidyverse)
mri_data_615 <- read_csv(paste0(base.location, "data/MRI_6.15.csv"))
mri_data <- mri_data_615 %>% dplyr::select(clinical_ptnum, pirads) %>% group_by(clinical_ptnum) %>% dplyr::summarize(pirads_max = max(pirads, na.rm = T))

mri_data$pirads_max <- ifelse(mri_data$pirads_max == "-Inf", NA, mri_data$pirads_max)
mri_data$id<-mri_data$clinical_ptnum

mri_data$subj<-rep(0,dim(mri_data)[1]) #because pt.data is ordered again, use the code that generates subj for subj2
for(i in 1:npat){
  mri_data$subj[mri_data$id==pt.data$id[i]] <- i}
...
tmp_dt <- pt.data %>% 
  ...
  left_join(mri_data_complete, by = c("clinical_PTnum" = "clinical_ptnum"))
ind_mri_data <- ifelse(is.na(tmp_dt$pirads_max), 0, 1)
