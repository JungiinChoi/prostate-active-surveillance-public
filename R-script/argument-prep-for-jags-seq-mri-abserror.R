...
inits <- function() {
  ...
  abserror_state <- pt.data$diff_eta_pgg[is.na(cancer_data)]
  layer_state <- pt.data$eta_ge_pgg[is.na(cancer_data)]
  ...
  abserror_slope <- rnorm(npred_abserror,mean=0,sd=0.25)
  abserror_coef_mean = rnorm(npred_abserror,mean=0,sd=0.25)
