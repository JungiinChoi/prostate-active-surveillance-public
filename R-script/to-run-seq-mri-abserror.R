...
location.of.r.scripts <- paste0(base.location, "R-script")
location.of.generated.files <- paste0(base.location, "generated-files-seq-mri-abserror")
...
# for(to.mask in 3:10){
 # to.mask = 1
load("~/Downloads/prostate-active-surveillance-vDaan/generated-files-seq-addmri/IOP-data-shaping-work-space.RData")
...
source(paste(location.of.r.scripts,"data-prep-for-jags-reform-seq-mri-abserror.R",sep="/"))

#Load arguments for JAGS
source(paste(location.of.r.scripts,"argument-prep-for-jags-seq-mri-abserror.R",sep="/"))
...
outj<-jags(jags_data, inits=inits, 
           ...
           model.file=paste(location.of.r.scripts, 
                            "JAGS-prediction-model-seq-mri-abserror.txt", sep="/"),
