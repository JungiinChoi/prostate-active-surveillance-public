...
jags_data<-list(nlevel_cancer=nlevel_cancer, nlevel_cancer_bin=nlevel_cancer_bin, npat=npat,
                ...
                pgg_pirads_data=pgg_pirads_data
...
inits <- function() {
  ...
  pgg_coef_mean[(npred_pgg+1):(npred_pgg+(nlevel_cancer))] = abs(pgg_coef_mean[(npred_pgg+1):(npred_pgg+(nlevel_cancer))])
  pgg_inv_sigma_sq = rgamma(1, 1, 1)
  pgg_slope1[(npred_pgg+1):(npred_pgg+(nlevel_cancer))] <- abs(pgg_slope1[(npred_pgg+1):(npred_pgg+(nlevel_cancer))])
  pgg_slope2[(npred_pgg+1):(npred_pgg+(nlevel_cancer))] <- abs(pgg_slope2[(npred_pgg+1):(npred_pgg+(nlevel_cancer))])
  pgg_slope3[(npred_pgg+1):(npred_pgg+(nlevel_cancer))] <- abs(pgg_slope3[(npred_pgg+1):(npred_pgg+(nlevel_cancer))])
