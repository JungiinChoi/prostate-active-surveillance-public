...
location.of.r.scripts <- paste0(base.location, "code-pom-mri-ordinal")
#location.of.r.scripts <- paste0(base.location, "10cv_Rscripts/pom-mri-ordinal/")
location.of.generated.files <- paste0(base.location, "generated-files-cv-pom-mri-ordinal")
...
# for(to.mask in 1:10){
  to.mask = 9
  load("generated-files-cv-pom-mri/IOP-data-shaping-work-space.RData")
...
#Define JAGS model
#source(paste(location.of.r.scripts,"cv-JAGS-prediction-model-jhpce.R",sep="/"))
...
outj<-jags(jags_data, inits=inits, 
           ...
           model.file=paste(location.of.r.scripts, 
                            "cv-JAGS-prediction-model-pom-mri-ordinal.txt", sep="/"),
