...
pt.data$eta_ge_pgg <- as.numeric(pt.data$true.pgg > pt.data$bx.pgg)


tmp_dt <- pt.data %>% 
  dplyr::select(subj2, clinical_PTnum, true.pgg, bx.pgg) %>% 
...
# binary_eta2 <- as.numeric(tmp_dt$true.pgg == 2)
# binary_eta3 <- as.numeric(tmp_dt$true.pgg == 3)
# binary_eta4 <- as.numeric(tmp_dt$true.pgg == 4)
# modmat_abserror <- cbind(binary_eta2, binary_eta3, binary_eta4, tmp_dt$have_mri)
...
abserror_data <- tmp_dt$diff_eta_pgg_plus1

npat_abserror <- sum(!is.na(abserror_data))
#abserror_patient_index_map <- tmp_dt$subj2
...
layer_state_map <- rep(NA, length(layer_state_index))
for(i in 1:length(layer_state_map)){
  layer_state_map[layer_state_index[i]]  = (1:length(layer_state_index))[i]
}
