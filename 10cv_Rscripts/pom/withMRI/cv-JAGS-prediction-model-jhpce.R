...
###PRIORS FOR BIOPSY OUTCOMES MODEL
...
pirads_int ~ dnorm(3.4, 1)T(0,)

#for(index in 1:npred_pirads) {
#  pirads_slope[index] ~ dnorm(xx, 1)T(0,)
#}

for(index in (npred_pirads + 1):(npred_pirads + (K - 1))) {
  pirads_slope[index] ~ dnorm(0.2, 1)T(0,)
}

resid_var_pirads ~ dunif(0, 1)
res_pirads <- pow(resid_var_pirads, -2)
...
### LINEAR MODEL FOR MRI PI-RADS SCORE
for(j in 1:npat_pirads){
	mu_pirads[j] <- pirads_int + 
	pirads_slope[(npred_pirads + 1)] * step(eta[pirads_patient_index_map[j]] - 2) + 
	pirads_slope[(npred_pirads + 2)] * step(eta[pirads_patient_index_map[j]] - 3) + 
	pirads_slope[(npred_pirads + 3)] * step(eta[pirads_patient_index_map[j]] - 4) 
	pirads_data[j] ~ dnorm(mu_pirads[j], res_pirads) 
}
  
 }", fill=TRUE, file=paste(location.of.r.scripts,"cv-JAGS-prediction-model-jhpce.txt", sep="/"))
