...
mri_data$subj2<-rep(0,dim(mri_data)[1]) #because pt.data is ordered again, use the code that generates subj for subj2
for(i in 1:n){
  mri_data$subj2[mri_data$id==pt.data$id[i]] <- i}

mri_data <- mri_data[mri_data$subj2 != 0,]
...
pirads_patient_index_map <- mri_data_complete$subj2
