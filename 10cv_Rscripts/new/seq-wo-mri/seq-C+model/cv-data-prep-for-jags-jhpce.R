...
bx.full$clinical_PTnum <- pt.data$clinical_PTnum[bx.full$subj]
...
names(mri_data) <- c("clinical_PTnum", "pirads_max")
bx.full <- bx.full %>% left_join(mri_data)
bx.full$have_mri <- ifelse(is.na(bx.full$pirads_max),0, 1)
...
pgg_have_mri <- as.matrix(c(pgg.data$have_mri, pgg.data2$have_mri))
## MRI data
library(tidyverse)
mri_data_615 <- read_csv("data/MRI_6.15.csv")
mri_data <- mri_data_615 %>% dplyr::select(clinical_ptnum, pirads) %>% group_by(clinical_ptnum) %>% summarize(pirads_max = max(pirads, na.rm = T))
