...
p_pgg1 <- expit(lin_pred1)

lin_pred2 <- alpha_med[2] + as.matrix(cbind(modmat_pgg, V.PGG.eta)) %*% as.vector(gamma_med[2,])
p_pgg2 <- 1/(1+exp(lin_pred1)) * expit(lin_pred2) 

lin_pred3 <- alpha_med[3] + as.matrix(cbind(modmat_pgg, V.PGG.eta)) %*% as.vector(gamma_med[3,])
p_pgg3 <- 1/(1+exp(lin_pred1))*1/(1+exp(lin_pred2))*expit(lin_pred3) 

p_pgg4 <- 1-p_pgg1-p_pgg2-p_pgg3

## plot
new.pgg.data <- rbind(pgg.data, pgg.data2)
new.pgg.data <- cbind(new.pgg.data, p_pgg1, p_pgg2, p_pgg3, p_pgg4)
library(ggplot2)
ggplot(new.pgg.data)+
  geom_line(aes(x=bx.dt.num, y=p_pgg1, group= subj, color="p_pgg1"), alpha = 0.3)+
  geom_line(aes(x=bx.dt.num, y=p_pgg2, group= subj, color="p_pgg2"), alpha = 0.3)+
  geom_line(aes(x=bx.dt.num, y=p_pgg3, group= subj, color="p_pgg3"), alpha = 0.3)+
  geom_line(aes(x=bx.dt.num, y=p_pgg4, group= subj, color="p_pgg4"), alpha = 0.3)
